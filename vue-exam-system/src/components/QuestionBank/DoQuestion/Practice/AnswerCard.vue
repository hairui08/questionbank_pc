...
const questionGroups = computed(() => {
  ...
  let runningNo = 1
  ordered.forEach(g => {
    g.questions = g.questions.map((q, i) => ({ ...q, displayNo: runningNo + i }))
    runningNo += g.questions.length
  })
...
.question-number.status-correct {
  background: var(--brand-primary, #52c41a);
  color: #ffffff;
  border-color: var(--brand-primary, #52c41a);
}
