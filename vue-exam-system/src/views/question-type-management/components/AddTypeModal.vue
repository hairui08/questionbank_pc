<template>
  <BaseModal
    :visible="isVisible"
    title="新增题型"
    description="配置题型在当前科目下的显示方式和排序"
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="form-group">
        ...
        <select
          ...
          :class="{ 'is-invalid': errors.internalType }"
          @change="errors.internalType = ''"
        >
          <option value="">请选择内部题型</option>
          <option
            v-for="option in INTERNAL_TYPE_OPTIONS"
            :key="option.value"
            :value="option.value"
          >
            {{ option.label }}
        ...
        <span v-if="errors.internalType" class="error-message">{{ errors.internalType }}</span>
      </div>
...
import type { QuestionTypeFormData, InternalType } from '../types'
import { INTERNAL_TYPE_OPTIONS } from '../types'
...
const emit = defineEmits<{
  ...
  submit: [data: QuestionTypeFormData]
}>()
...
const validate = (): boolean => {
  ...
  // 验证内部题型
  if (!formData.internalType) {
    errors.internalType = '请选择内部题型'
    isValid = false
  }
