<template>
  <AppLayout title="题库系统分类管理">
    <TabNavigation :tabs="tabs" :default-tab="'prototype'">
      ...
      <template #prototype>
        <div class="tab-panel">
          ...
          @batch-toggle-section-status="handleBatchToggleSectionStatus"
          @batch-toggle-chapter-status="handleBatchToggleChapterStatus"
        />
          </div>
        </div>
      </template>
      ...
      <template #requirements>
        <div class="tab-panel">
          <div class="requirements-header">
            <h2>分类管理需求文档</h2>
            <p>详细的功能需求、业务规则和验收标准，确保分类管理模块实现标准化。</p>
          </div>
    ...
    <AddSectionModal
      ...
      :section-id="addingSubSectionSection?.id || ''"
      :section-name="addingSubSectionSection?.name || ''"
      :default-order="addingSectionChapter ? getNextSectionOrder(addingSectionChapter.id) : getNextSubSectionOrder(addingSubSectionSection?.id || '')"
...
const editingSubSection = ref<Section | null>(null)
const addingSectionChapter = ref<Chapter | null>(null)
const addingSubSectionSection = ref<Section | null>(null)
...
const toggleEntityType = ref<'chapter' | 'section' | 'batch-section'>('chapter')
const pendingToggleChapter = ref<Chapter | null>(null)
const pendingToggleSection = ref<Section | null>(null)

// 批量操作状态
const pendingBatchSectionIds = ref<string[]>([])
const pendingBatchTargetStatus = ref<'active' | 'disabled'>('disabled')
 ...
 * 获取下一个第三级子项排序号
 */
const getNextSubSectionOrder = (sectionId: string): number => {
  const subSections = chapterStore.getSubSectionsBySection(sectionId).value
  return subSections.length > 0 ? Math.max(...subSections.map((s) => s.order)) + 1 : 1
}
 ...
 * 打开添加小节弹窗
 */
const openAddSectionModal = (parent: Chapter | Section) => {
  if ('subjectId' in parent) {
    // 父级是章节，添加第二级小节
    addingSectionChapter.value = parent as Chapter
    addingSubSectionSection.value = null
  } else {
    // 父级是小节，添加第三级子项
    addingSubSectionSection.value = parent as Section
    addingSectionChapter.value = null
  }
...
const handleAddSection = (data: any) => {
  try {
    if (data.sectionId) {
      // 添加第三级子项
      chapterStore.addSubSection(data)
      showToast('第三级子项添加成功', { type: 'success' })
    } else {
      // 添加第二级小节
 ...
 * 批量切换小节状态 - 显示确认弹窗
 */
const handleBatchToggleSectionStatus = (
  sectionIds: string[],
  targetStatus: 'active' | 'disabled'
) => {
  if (sectionIds.length === 0) return

  toggleEntityType.value = 'batch-section'
  pendingBatchSectionIds.value = sectionIds
  pendingBatchTargetStatus.value = targetStatus

  const actionText = targetStatus === 'disabled' ? '禁用' : '启用'
  toggleActionType.value = targetStatus === 'disabled' ? 'disable' : 'enable'
  toggleMessage.value = `确定要批量${actionText} ${sectionIds.length} 个小节吗？`
 ...
 * 批量切换章节状态 - 显示确认弹窗
 */
const handleBatchToggleChapterStatus = (
  chapterIds: string[],
  targetStatus: 'active' | 'disabled'
) => {
  if (chapterIds.length === 0) return

  toggleEntityType.value = 'batch-section' // 使用相同的弹窗类型
  pendingBatchSectionIds.value = chapterIds // 复用相同的变量存储章节ID
  pendingBatchTargetStatus.value = targetStatus

  const actionText = targetStatus === 'disabled' ? '禁用' : '启用'
  toggleActionType.value = targetStatus === 'disabled' ? 'disable' : 'enable'
  toggleMessage.value = `确定要批量${actionText} ${chapterIds.length} 个章节吗？`
  ...
  const handleToggleStatusConfirm = () => {
    try {
      if (toggleEntityType.value === 'chapter' && pendingToggleChapter.value) {
        ...
        isToggleStatusModalVisible.value = false
      } else if (toggleEntityType.value === 'batch-section' && pendingBatchSectionIds.value.length > 0) {
        // 区分批量操作类型：章节还是小节
        const isChapterBatch = pendingBatchSectionIds.value.some(id => {
          // 检查是否存在于章节列表中
          return chapterStore.chapters.some(chapter => chapter.id === id)
        })

        if (isChapterBatch) {
          // 批量更新章节状态
          chapterStore.batchToggleChapters(pendingBatchSectionIds.value, pendingBatchTargetStatus.value)
          const actionText = pendingBatchTargetStatus.value === 'disabled' ? '禁用' : '启用'
          showToast(`已成功批量${actionText} ${pendingBatchSectionIds.value.length} 个章节`, { type: 'success' })
        } else {
          // 批量更新小节状态
          chapterStore.batchToggleSectionStatus(pendingBatchSectionIds.value, pendingBatchTargetStatus.value)
          const actionText = pendingBatchTargetStatus.value === 'disabled' ? '禁用' : '启用'
          showToast(`已成功批量${actionText} ${pendingBatchSectionIds.value.length} 个小节`, { type: 'success' })
        }

        isToggleStatusModalVisible.value = false
        // 清空批量操作状态
        pendingBatchSectionIds.value = []
      }
    } catch (error) {
      showToast((error as Error).message, { type: 'error' })
    }
