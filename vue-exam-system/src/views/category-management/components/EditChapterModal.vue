...
watch(
  ...
  (newVal) => {
    ...
    if (newVal && props.chapter) {
      // 填充表单数据
      formData.projectId = props.chapter.projectId
      formData.projectName = props.chapter.projectName
      ...
      // 如果名称是"章节练习"，则设置为true
      formData.isChapterPractice = props.chapter.name === '章节练习' || false
      // 根据章节名称设置初始类型
      if (props.chapter.name === '章节练习') {
        formType.value = 'chapter-practice'
      } else if (props.chapter.name === '历年真题') {
        formType.value = 'past-exam'
      } else {
