...
const emit = defineEmits<{
  ...
  submit: [data: KnowledgePointFormData]
}>()
...
const chaptersTree = computed(() => {
  ...
  const chapterList = chapterStore.getChaptersBySubject(props.activeSubject.id).value
...
watch(
  ...
  (newVal) => {
    ...
    if (newVal) {
      // 重置表单并自动填充科目ID
      formData.subjectId = props.activeSubject?.id || ''
      formData.name = ''
      formData.chapterIds = []
      errors.name = ''
      // 重置章节展开状态
