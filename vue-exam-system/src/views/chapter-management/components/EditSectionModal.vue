  ...
  >
    <form @submit.prevent="handleSubmit">
      <div class="form-group">
        <label for="chapter">所属章节 <span class="required">*</span></label>
        <input
          id="chapter"
      ...
      <div class="form-group">
        <label for="section-name">小节名称 <span class="required">*</span></label>
        <input
          id="section-name"
          v-model="formData.name"
          type="text"
          placeholder="请输入小节名称（1-50字符）"
          maxlength="50"
      ...
      <div class="form-group">
        <label for="section-status">状态 <span class="required">*</span></label>
        <select id="section-status" v-model="formData.status">
...
const emit = defineEmits<{
  ...
  submit: [sectionId: string, data: Partial<SectionFormData>]
}>()
...
watch(
  ...
  (newVal) => {
    ...
    if (newVal && props.section) {
      // 填充表单数据
      formData.chapterId = props.section.chapterId
      formData.chapterName = props.section.chapterName
      formData.name = props.section.name
      formData.status = props.section.status
      formData.order = props.section.order
      errors.name = ''
      errors.order = ''
    }
  }
