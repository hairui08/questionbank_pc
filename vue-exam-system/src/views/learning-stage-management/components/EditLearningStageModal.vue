...
import type { LearningStage, LearningStageFormData } from '../types'
...
const emit = defineEmits<{
  ...
  submit: [id: string, data: LearningStageFormData]
}>()
...
watch(
  ...
  (newVal) => {
    ...
    if (newVal && props.stage) {
      // 填充表单数据
      formData.name = props.stage.name
      formData.description = props.stage.description || ''
      errors.name = ''
      errors.description = ''
    }
  }
 ...
 */
const handleSubmit = () => {
  if (!validate() || !props.stage) {
    return
  }

  emit('submit', props.stage.id, {
